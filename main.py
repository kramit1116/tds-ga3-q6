...
async def answer_audio(request: Request):
    ...
    try:
        ...
        # FIX: Force strip spaces from all column names
        columns = [str(c).replace(" ", "") for c in (ext.get("columns", []) or [])]
        data_rows = ext.get("data_rows", []) or []
        req_stats = ext.get("requested_stats", [])
        num_rows = ext.get("num_rows")
        
        # FIX: Force strip spaces from all keys inside explicit_stats
        raw_explicit_stats = ext.get("explicit_stats", {})
        explicit_stats = {}
        for stat_name, stat_val in raw_explicit_stats.items():
            if isinstance(stat_val, dict):
                explicit_stats[stat_name] = {str(k).replace(" ", ""): v for k, v in stat_val.items()}
            elif isinstance(stat_val, list):
                # Fix correlation lists
                fixed_list = []
                for item in stat_val:
                    if isinstance(item, dict):
                        if "x" in item: item["x"] = str(item["x"]).replace(" ", "")
                        if "y" in item: item["y"] = str(item["y"]).replace(" ", "")
                    fixed_list.append(item)
                explicit_stats[stat_name] = fixed_list
            else:
                explicit_stats[stat_name] = stat_val
